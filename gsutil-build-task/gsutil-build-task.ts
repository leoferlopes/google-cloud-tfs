...
/**
 * @fileoverview This module holds the logic for the gsutil-build-task.
 */
...
export interface RunGsutilOptions {
  gsutilTool: ToolRunner;
  endpoint: Endpoint;
  command: string;
...
export function runGsutil(runOptions: RunGsutilOptions): void {
  const command = runOptions.command.trim().replace(/^gsutil\s*/, '');
  runOptions.gsutilTool.line(command)
      .arg(Endpoint.credentialParam)
      .argIf(runOptions.includeProjectParam, `-p "${runOptions.endpoint.projectId}"`);
